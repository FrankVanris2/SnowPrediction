...
Date: 4/12/2024
Desc: Creating a dataframe with all of the needed testData to be sent into the KNN learning model
'''

#my imports
import pandas as pd

#creating a function that will contain the historical testData needed for the KNN algorithm
    #obtaining historical testData
listHistoricalData = []
for year in range(2016,2023):
    historicalData = pd.read_csv("testData\\SnoqualmiePass_3010_" + str(year) + ".csv")

    #removing Solar Radiation from the 2016 testData since it's the only one that has it
    if year == 2016:
        historicalData = historicalData.drop('Solar Radiation  (W/m2) ', axis='columns')

    #reducing 1 column from each of our testData since we don't need to know the total snow depth, Battery Voltage, and Solar Radiation
    ...
    #reverse the testData to Jan 1st to Dec 31st
